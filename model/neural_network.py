...
class cnn_strain_model(nn.Module):
    def __init__(self):
        super(cnn_strain_model, self).__init__()
        self.d_prob = 0.5
...
class trainNN():
    ...
    def train(self,trainX,trainY,testX,testY,cal_test_accuracy=True,path='model/kl_strain.pt'):
        ...
        for epoch in range(self.epoch):
            ...
            for i in range(num_training // self.batch_size):
                ...
                Y_ = trainY[i * self.batch_size:(i + 1) * self.batch_size]
                self.nn_model.zero_grad()
                y_predicted = self.nn_model.forward(X_)
    ...
    def cal_accuracy(self,testX,testY):
        n_test = len(testX)
        self.nn_model.eval()
        self.nn_model.zero_grad()
        avg_loss = 0
...
class reward_model():
    ...
    def predict_strain(self,X_data):
        self.nn_model.eval()
        self.nn_model.zero_grad()
        with torch.no_grad():
